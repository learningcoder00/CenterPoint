...
它不只是一个“把推理结果画出来”的工具，而是一个更贴近企业真实工作流的分析平台。

在很多企业自动驾驶或具身智能团队中，真正困难的往往不是“能不能跑一次推理”，而是下面这些更长期的问题：

1. clip 资产分散在不同目录、不同脚本和不同人的经验里，难以集中管理。
2. 不同工程师对问题 clip 的命名、分类和记录方式不一致，难以形成规范化分析流程。
3. corner case 常常只停留在“看到了一个问题”的层面，缺少可复现、可追踪、可沉淀的诊断闭环。
4. 算法推理、结果可视化、问题解释和模型改进建议往往彼此割裂，难以服务真正的迭代效率。

本项目的目标，就是把这些分散环节串成一条完整链路：先把 clip 变成可以被管理和检索的对象，再把可视化任务变成标准化流程，最后通过 AI 模块把观察到的问题转化为更具操作性的模型改进建议。
...
一家企业的感知团队已经积累了大量路测 clip，但这些 clip 分散在文件夹和脚本里。每次有人发现新的 corner case，往往只能靠截图、口头描述或临时记录来沟通。久而久之，团队会遇到几个典型痛点：

- 同一个问题场景被重复分析，但没有统一沉淀。
- 关键 clip 找不到，或者找到后也无法快速完成统一可视化。
- 发现了模型误检、漏检、遮挡、密集交通、夜晚强反光等问题，却没有稳定的方式把这些问题转成下一轮改进输入。

而本系统希望做的，就是把“零散的问题收集”变成“规范化的分析闭环”：

1. 用 clip 视图把样本资产整理起来。
2. 用 tags 和搜索把 corner case 变成可检索、可复用的分析对象。
3. 用可视化任务把模型输出转成可读、可共享的视频证据。
4. 用结果页把执行状态、日志和视频统一管理。
5. 用 AI 模块根据任务结果和问题描述，生成更接近研发语言的优化建议，帮助团队更快进入下一轮模型改进。

因此，这个项目本质上服务的不是一次性的可视化，而是企业内部对 clip 的持续管理能力、对问题场景的标准化分析能力，以及对 corner case 的系统化消化能力。
...
- 基于任务结果与用户问题描述生成 AI 建议
- 记录 AI 优化历史，便于团队复盘
- 将“发现问题”进一步推进到“提出改进方向”

这一层不是替代算法工程师，而是帮助团队把可视化分析后的观察结果更快整理成结构化建议，例如：

- 这个 corner case 更像是数据覆盖不足，还是模型表达能力不足
- 是否需要补充某类 tag 对应的数据
- 是否应优先排查阈值、NMS、类别混淆、时序稳定性或特征表达问题
- 哪些问题更适合通过数据回流解决，哪些更适合通过模型结构或训练策略调整解决
...
- `Clips`: 样本资产管理与问题片段筛选
- `Results`: 任务执行与结果证据沉淀
- `AI Optimization`: 从结果走向诊断与改进建议

也就是说，页面设计本身就在服务一个完整闭环，而不是若干互不相关的功能页面。
